package com.mainframevampire.ryan.wheretobuy.model;

import com.mainframevampire.ryan.wheretobuy.R;

public class Swisse {
    public static final String[] id = new String[]{
                "SWS001",
                "SWS002",
                "SWS003",
                "SWS004",
                "SWS005",
                "SWS006",
                "SWS007",
                "SWS008",
                "SWS009",
                "SWS010",
                "SWS011",
                "SWS012",
                "SWS013",
                "SWS014",
                "SWS015",
                "SWS016",
                "SWS017",
                "SWS018",
                "SWS019",
                "SWS020",
                "SWS021",
                "SWS022",
                "SWS023",
                "SWS024",
                "SWS025",
                "SWS026",
                "SWS027",
                "SWS028",
                "SWS029",
                "SWS030",
                "SWS031",
                "SWS032",
                "SWS033",
                "SWS034",
                "SWS035",
                "SWS036",
                "SWS037",
                "SWS038",
                "SWS039",
                "SWS040",
                "SWS041"};

    public static final String[] shortName = new String[]{
            "SWS Chanberry 25000mg 30Caps",
            "SWS High Chanberry 90Caps   ",
            "SWS Celery 5000mg 50Caps    ",
            "SWS Liver Detox 120Tabs     ",
            "SWS Liver Detox 200Tabs     ",
            "SWS Liver Detox 60Tabs      ",
            "SWS Cal+Vitamin D 90Tabs    ",
            "SWS Cal+Vitamin D 150Tabs   ",
            "SWS Evening Oil 200Caps     ",
            "SWS Memory + Focus 50Tabs   ",
            "SWS Women's Ultivite 60Tabs ",
            "SWS Women's Ultivite 120Tabs",
            "SWS Women's Ultivite 50+ 90T",
            "SWS Men Ultivite 60Tabs     ",
            "SWS Men Ultivite 120Tabs    ",
            "SWS Men Ultivite 50+ 90Tabs ",
            "SWS Lecithin 1200mg 150Caps ",
            "SWS Lecithin 1200mg 300Caps ",
            "SWS Lung Support 90Tabs     ",
            "SWS Mega B + 60 Tablets     ",
            "SWS Organic Spirulina 100T  ",
            "SWS High Spirulina 1kmg 200T",
            "SWS Chlorophyll 100Tabs     ",
            "SWS Chlorophyll 1000mg 200T ",
            "SWS Chlorophyll Berry 500ml ",
            "SWS Chlorophyll Spearmint500",
            "SWS High Vitamin C 60Tabs   ",
            "SWS Relax&Sleep 60Tabs      ",
            "SWS Krill Oil 500mg 60Caps  ",
            "SWS Gluco Sulfate 1500mg 180",
            "SWS Menopause Balance 60Tabs",
            "SWS Propolis 2000mg 210Tabs ",
            "SWS Hunger Control 50Tabs   ",
            "SWS Mood 100Tabs            ",
            "SWS Child  FishOil 90Caps   ",
            "SWS Grape Seed 180Tabs      ",
            "SWS Children's Ulti 120tabs ",
            "SWS Prostate 50 Tablets     ",
            "SWS Ultiboost Sleep 100Tabs ",
            "SWS Hair Skin Nails 500ml   ",
            "SWS Hair Skin Nails+ 100Tabs"};

    public static final String[] longName = new String[] {
            "Swisse High Strength Cranberry 25,000mg 30 Caps",
            "Swisse High Strength Cranberry 90 Capsules",
            "Swisse High Strength Celery 5000 mg 50 Caps",
            "Swisse Ultiboost Liver Detox 120 Tablets",
            "Swisse Ultiboost Liver Detox 200 Tablets",
            "Swisse Ultiboost Liver Detox 60 Tablets",
            "Swisse Ultiboost Calcium + Vitamin D 90 tabs",
            "Swisse Ultiboost Calcium + Vitamin D 150 tabs",
            "Swisse Ultiboost Evening Primrose Oil 200 Caps",
            "Swisse Ultiboost Memory + Focus 50 Tablets",
            "Swisse Women's Ultivite 60 Tablets",
            "Swisse Women's Ultivite 120 Tablets",
            "Swisse Women's Ultivite 50+ 90 Tablets",
            "Swisse Men's Ultivite 60 Tablets",
            "Swisse Men's Ultivite 120 Tablets",
            "Swisse Men's Ultivite 50+ 90 Tablets",
            "Swisse Ultiboost Lecithin 1200mg 150 Caps",
            "Swisse Ultiboost Lecithin 1200mg 300 Capsules",
            "Swisse Ultiboost Lung Health Support 90 Tablets",
            "Swisse Ultiboost Mega B + 60 Tablets",
            "Swisse Organic Spirulina 100 Tablets",
            "Swisse High Strength Organic Spirulina 200 Tabs",
            "Swisse Chlorophyll+ 100 Tablets",
            "Swisse High Strength Chlorophyll+  200 Tablets",
            "Swisse Chlorophyll Mixed Berry 500ml",
            "Swisse Chlorophyll Spearmint 500ml",
            "Swisse Ultiboost High Strength VC 60 Tabs",
            "Swisse Ultiboost Relax & Sleep 60 Tablets",
            "Swisse Ultiboost Deep Sea Krill Oil 60 Caps",
            "Swisse Ultiboost Glucosamine Sulfate  180 Tabs ",
            "Swisse Ultiboost Menopause Balance 60 Tablets",
            "Swisse Ultiboost High Strength Propolis 210Tabs",
            "Swisse Ultiboost Hunger Control 50 Tablets",
            "Swisse Ultiboost Mood 100 Tablets",
            "Swisse Children's Smart Fish Oil 90 Caps",
            "Swisse Ultiboost Grape Seed 180 Tablets",
            "Swisse Children's Ultivite 120 Tablets",
            "Swisse Ultiboost Prostate 50 Tablets",
            "Swisse Ultiboost Sleep 100 Tablets",
            "Swisse Ultiboost Hair Skin Nails 500ml",
            "Swisse Ultiboost Hair Skin Nails+ 100 Tablets"
    };

    public static float[] lowestPrice = new float[id.length];
    public static float[] highestPrice = new float[id.length];
    public static String[] whichIsLowest = new String[id.length];

    public static final String[] cmwID = new String[]{
            "67497",
            "80611",
            "74932",
            "66552",
            "80460",
            "59769",
            "     ",
            "67491",
            "70258",
            "57049",
            "31587",
            "52658",
            "58346",
            "31581",
            "55511",
            "58347",
            "74931",
            "80463",
            "77526",
            "67490",
            "63081",
            "67499",
            "63448",
            "67492",
            "67493",
            "58504",
            "74651",
            "67970",
            "66521",
            "75063",
            "70153",
            "73682",
            "66537",
            "66392",
            "65702",
            "73685",
            "66553",
            "56439",
            "58108",
            "63080",
            "58348"};

    public static float[] cmwPrice = new float[id.length];

    public static final String[] plID = new String[] {
                "14249",
                "     ",
                "13547",
                "14749",
                "     ",
                "13756",
                "13568",
                "13574",
                "14254",
                "14079",
                "14259",
                "14237",
                "14239",
                "14105",
                "14094",
                "14097",
                "13753",
                "     ",
                "18255",
                "14168",
                "13854",
                "13860",
                "16328",
                "13758",
                "13798",
                "16325",
                "     ",
                "14169",
                "     ",
                "     ",
                "14252",
                "14118",
                "14344",
                "14164",
                "     ",
                "14022",
                "13628",
                "14096",
                "14158",
                "14037",
                "14034"};

    public static float[] plPrice = new float[id.length];

    public static final String[] flID = new String[] {
            "10118",
            "     ",
            "13477",
            "12782",
            "     ",
            "7665 ",
            "     ",
            "10229",
            "13714",
            "14419",
            "11124",
            "6451 ",
            "     ",
            "2406 ",
            "6450 ",
            "7547 ",
            "13713",
            "     ",
            "14339",
            "10226",
            "8214 ",
            "14885",
            "14056",
            "14070",
            "     ",
            "     ",
            "9935 ",
            "10411",
            "     ",
            "13603",
            "13723",
            "14102",
            "     ",
            "11196",
            "13728",
            "13724",
            "13727",
            "2407 ",
            "7667 ",
            "9688 ",
            "6626 "};

    public static float[] flPrice = new float[id.length];

    public static final String[] twID = new String[] {
            "37601",
            "     ",
            "43695",
            "35227",
            "     ",
            "31371",
            "35941",
            "41529",
            "43790",
            "35919",
            "30674",
            "30969",
            "31243",
            "30787",
            "30968",
            "31242",
            "42988",
            "     ",
            "43595",
            "46752",
            "31247",
            "46753",
            "31167",
            "     ",
            "43611",
            "31043",
            "36167",
            "42173",
            "     ",
            "42987",
            "43628",
            "43656",
            "35226",
            "     ",
            "36132",
            "43817",
            "35219",
            "31470",
            "31253",
            "31269",
            "31248"};

    public static float[] twPrice = new float[id.length];

    public static final String[] hwID = new String[] {
            "377791",
            "      ",
            "      ",
            "354392",
            "      ",
            "311685",
            "      ",
            "379395",
            "397318",
            "      ",
            "447161",
            "275093",
            "301566",
            "447293",
            "275107",
            "301574",
            "      ",
            "      ",
            "      ",
            "      ",
            "      ",
            "      ",
            "336394",
            "      ",
            "379352",
            "306649",
            "      ",
            "287105",
            "      ",
            "      ",
            "      ",
            "893773",
            "354422",
            "      ",
            "352071",
            "893765",
            "      ",
            "287083",
            "301558",
            "335770",
            "301531"};

        public static float[] hwPrice = new float[id.length];

        public static int getSwisseImageId(String id){
            int swisseImageId = R.drawable.sws001;

            switch (id) {
                case "SWS001":
                    swisseImageId = R.drawable.sws001;
                    break;
                case "SWS002":
                    swisseImageId = R.drawable.sws002;
                    break;
                case "SWS003":
                    swisseImageId = R.drawable.sws003;
                    break;
                case "SWS004":
                    swisseImageId = R.drawable.sws004;
                    break;
                case "SWS005":
                    swisseImageId = R.drawable.sws005;
                    break;
                case "SWS006":
                    swisseImageId = R.drawable.sws006;
                    break;
                case "SWS007":
                    swisseImageId = R.drawable.sws007;
                    break;
                case "SWS008":
                    swisseImageId = R.drawable.sws008;
                    break;
                case "SWS009":
                    swisseImageId = R.drawable.sws009;
                    break;
                case "SWS010":
                    swisseImageId = R.drawable.sws010;
                    break;
                case "SWS011":
                    swisseImageId = R.drawable.sws011;
                    break;
                case "SWS012":
                    swisseImageId = R.drawable.sws012;
                    break;
                case "SWS013":
                    swisseImageId = R.drawable.sws013;
                    break;
                case "SWS014":
                    swisseImageId = R.drawable.sws014;
                    break;
                case "SWS015":
                    swisseImageId = R.drawable.sws015;
                    break;
                case "SWS016":
                    swisseImageId = R.drawable.sws016;
                    break;
                case "SWS017":
                    swisseImageId = R.drawable.sws017;
                    break;
                case "SWS018":
                    swisseImageId = R.drawable.sws018;
                    break;
                case "SWS019":
                    swisseImageId = R.drawable.sws019;
                    break;
                case "SWS020":
                    swisseImageId = R.drawable.sws020;
                    break;
                case "SWS021":
                    swisseImageId = R.drawable.sws021;
                    break;
                case "SWS022":
                    swisseImageId = R.drawable.sws022;
                    break;
                case "SWS023":
                    swisseImageId = R.drawable.sws023;
                    break;
                case "SWS024":
                    swisseImageId = R.drawable.sws024;
                    break;
                case "SWS025":
                    swisseImageId = R.drawable.sws025;
                    break;
                case "SWS026":
                    swisseImageId = R.drawable.sws026;
                    break;
                case "SWS027":
                    swisseImageId = R.drawable.sws027;
                    break;
                case "SWS028":
                    swisseImageId = R.drawable.sws028;
                    break;
                case "SWS029":
                    swisseImageId = R.drawable.sws029;
                    break;
                case "SWS030":
                    swisseImageId = R.drawable.sws030;
                    break;
                case "SWS031":
                    swisseImageId = R.drawable.sws031;
                    break;
                case "SWS032":
                    swisseImageId = R.drawable.sws032;
                    break;
                case "SWS033":
                    swisseImageId = R.drawable.sws033;
                    break;
                case "SWS034":
                    swisseImageId = R.drawable.sws034;
                    break;
                case "SWS035":
                    swisseImageId = R.drawable.sws035;
                    break;
                case "SWS036":
                    swisseImageId = R.drawable.sws036;
                    break;
                case "SWS037":
                    swisseImageId = R.drawable.sws037;
                    break;
                case "SWS038":
                    swisseImageId = R.drawable.sws038;
                    break;
                case "SWS039":
                    swisseImageId = R.drawable.sws039;
                    break;
                case "SWS040":
                    swisseImageId = R.drawable.sws040;
                    break;
                case "SWS041":
                    swisseImageId = R.drawable.sws041;
                    break;
            }

            return swisseImageId;
        }

    
}
